// setValues.h --- 
// 
// Filename: setValues.h
// Description: 
// 
// Author:    Yu Lu
// Email:     yulu@utexas.edu
// Github:    https://github.com/SuperYuLu 
// 
// Created: Sun Apr 30 18:45:23 2017 (-0500)
// Version: 
// Last-Updated: Wed May  3 23:34:41 2017 (-0500)
//           By: yulu
//     Update #: 110
//

#ifndef SETVALUE_H
#define SETVALUE_H

extern int const excitedF = 2;
extern double const pi = 3.1415926;
extern double const intensityRatio = 1;
extern double const gamma = 36.898e6; // From 6Li D1/D2 line
extern double const plankConst = 1.054571e-34;
extern double const lifeTime = 27e-9; // [s]
extern double const saturationLightInten = 2.56; // [mW /cm^2]
extern double const lightInten = 0.000002; // [mW / cm^2]
extern double const tStep = 5e-9; // [s]
extern double const totTime = 25e-6; // [s]
extern double const dipoleElement = 3.977e-29;  // [C dot m] For 6Li D2 line
//extern double const dipoleElement = 2.537e-29;  // For Rb87

#endif

